...
# `apps/workers` — pg-boss handler process. Same workspace install layer
# as the api Dockerfile; different CMD.
#
...
RUN pnpm --filter @aisecretary/workers typecheck
...
USER aisecretary
CMD ["pnpm", "--filter", "@aisecretary/workers", "exec", "tsx", "src/index.ts"]
